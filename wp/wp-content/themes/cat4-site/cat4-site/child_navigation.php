...
$page_exclusions = ''; // enter any excluded pages
global $post;
if($post->ID) {
			...
			if($grandparent->post_parent) { // if page is a greatgrandchild
				...
				if($greatgrandparent->post_parent) { // if page is a greatgreatgrandchild
					...
					if($greatgreatgrandparent->post_parent) { // if page is a greatgreatgreatgrandchild
						...
						$section_overview_link = get_permalink($greatgreatgreatgrandparent->ID);
						$section_overview = '<li><a href="'.get_permalink($greatgreatgreatgrandparent->ID).'">'.$section_overview_title.'</a></li>';
						$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$greatgreatgrandparent->post_parent.'&depth=1&echo=0');
					}
					else {
						$section_title = get_the_title($greatgreatgrandparent->ID);
						$section_overview_link = get_permalink($greatgreatgrandparent->ID);
						$section_overview = '<li><a href="'.get_permalink($greatgreatgrandparent->ID).'">'.$section_overview_title.'</a></li>';
						$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$greatgrandparent->post_parent.'&depth=1&echo=0');
					}
				}
				else {
					$section_title = get_the_title($greatgrandparent->ID);
					$section_overview_link = get_permalink($greatgrandparent->ID);
					$section_overview = '<li><a href="'.get_permalink($greatgrandparent->ID).'">'.$section_overview_title.'</a></li>';
					$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$grandparent->post_parent.'&depth=1&echo=0');
				}	
			}
				...
				$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$parent->post_parent.'&depth=1&echo=0');
			}
		}
		else { // page off of secondary nav has children
			$section_title = get_the_title($parent->ID);
			$section_overview_link = get_permalink($parent->ID);
			$section_overview = '<li><a href="'.get_permalink($parent->ID).'">'.$section_overview_title.'</a></li>';
			$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$post->post_parent.'&depth=1&echo=0');
		}
	}
	else {
		if(wp_list_pages("child_of=".$post->ID."&depth=1&echo=0")) { // if page off of main nav has children
			//$section_title = get_the_title($post->ID);
			$section_overview_link = get_permalink($post->ID);
			$section_overview = '<li class="current_page_item"><a href="'.get_permalink($post->ID).'">'.$section_overview_title.'</a></li>';
			$children = wp_list_pages('sort_column=menu_order&exclude='.$page_exclusions.'&title_li=&child_of='.$post->ID.'&depth=1&echo=0');
		}
	}
}
?>