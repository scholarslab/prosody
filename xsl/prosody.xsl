                        ...
                        <xsl:for-each select="TEI:seg">
                              ...
                              <xsl:for-each select="text()|*/text()">
                                    ...
                                    <xsl:for-each select="tokenize(string(.),' ')">
                                          <xsl:if test="string(.)">
<span rev="231" class="prosody-shadowsyllable" shadow="" id="prosody:shadow:{$line-number}:{$seg-position}:{$foot-position}:{position()}" onclick="switchstress(this);"><span class="prosody-placeholder"><xsl:copy-of select="string(.)"/></span></span>
                                          </xsl:if>
                                    </xsl:for-each>
                              </xsl:for-each>
                        </xsl:for-each>
